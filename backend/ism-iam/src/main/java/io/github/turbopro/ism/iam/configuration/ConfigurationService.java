...
public class ConfigurationService {
    ...
    @Transactional
    public ConfigurationModels.SettingView updateSetting(String key,ConfigurationModels.UpdateSetting command){long tenantId=tenant();String type=systemMapper.settingType(key);if(type==null)throw new ApiException(ConfigurationErrorCode.UNKNOWN_SETTING);validate(type,command.value());Integer current=mapper.tenantSettingVersion(tenantId,key);if(current==null){if(command.version()!=0)throw new ApiException(CommonErrorCode.CONFLICT);mapper.insertSetting(tenantId,ids.nextId(),key,type,command.value());}else if(current!=command.version()||mapper.updateSetting(tenantId,key,command.value(),command.version())!=1)throw new ApiException(CommonErrorCode.CONFLICT);return settings().stream().filter(item->item.key().equals(key)).findFirst().orElseThrow();}
