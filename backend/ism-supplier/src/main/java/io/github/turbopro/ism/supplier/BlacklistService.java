...
public class BlacklistService {
    ...
    @Transactional public BlacklistModels.View create(BlacklistModels.Create command){
        ...
        if(mapper.openCase(i.tenantId(),supplierId)>0)throw new ApiException(CommonErrorCode.CONFLICT,"该供应商已有未结束的黑名单申请");
        long caseId=ids.nextId();mapper.insert(caseId,i.tenantId(),supplierId,Long.parseLong(supplier.organizationId()),supplier.code(),supplier.name(),command.reason().trim(),blank(command.sourceRef()),i.actorId());
